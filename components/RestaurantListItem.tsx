    ...
    <div className="w-[327px] h-[202px] bg-white border-munchies-gray rounded-lg shadow-sm relative overflow-hidden flex flex-col justify-between p-4">
      <div>
        ...
        <Image
          ...
          quality={100}
          alt="Card image"
          className={`absolute -top-9 -right-4 object-cover ${!isOpen ? 'grayscale opacity-40' : ''}`}
        />
      </div>
      <div className="flex flex-row justify-between content-center">
        <p className={`text-2xl overflow-hidden text-ellipsis ${!isOpen ? 'opacity-40' : ''}`}>{restaurant.name}</p>
        <Button
          variant="default"
          disabled={!isOpen}
